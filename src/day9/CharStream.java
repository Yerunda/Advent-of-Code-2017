...
public class CharStream {
	...
	public void run(){
		...
		for(int i = 0; i < stream.size(); i++){
			switch(stream.get(i)){
				case GROUP_OPEN: 	if(!garbage){
										nLvl++;
									}else{
										garbageCount++;
									}
					break;
				case GROUP_CLOSE: 	if(!garbage){
										score += nLvl;
										nLvl--;
									}else{
										garbageCount++;
					...
					break;
				case GARBAGE_OPEN: 	if(garbage){
										garbageCount++;
									}else{
										garbage = true;
									}
					break;
				case GARBAGE_CLOSE: garbage = false;
					break;
				case NEXT_IGNORE: 	i++;
					break;
				default:			garbageCount = garbage ? garbageCount + 1 : garbageCount;
					break;
			}
		}
